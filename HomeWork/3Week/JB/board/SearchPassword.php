<?php
	...
	if(!$result)
	{
		...
		while ($row = mysql_fetch_array($result)) {
			...
			if($row['id']== $id && $row['name'] ==$name && $row['social_security_number'] ==$social_security_number )
			{
				echo '찾으시는 비밀번호는'.'</br>'.$row['password'].'입니다 '.'</br>'.
		...
		if(!($ok))
		{
		echo '찾으시는 비밀번호가 없습니다'.'</br>'.'다시입력하여주십시오'.'</br>'.
